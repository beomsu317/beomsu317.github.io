...
가상 메모리의 주소로 전달할 경우 shellcode가 실행되지 않는다. `KERN_EXEC_LOW`, `KERN_EXEC_HIGH`를 0xffff880001bfffffUL, 0xffff880036000000UL로 변경한다. 변경하지 않을 경우 physical 영역의 권한 문제로 root를 획득하지 못하는 경우가 발생한다.
